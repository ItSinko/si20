...
if( isset( $_REQUEST['aksi'] )){
		...
		switch($aksi){
			case 'baru':
				include 'gudang/tambah.php';
				break;
				
			case 'edit':
				include 'gudang/edit.php';
				break;
				
			case 'delete':
				include 'gudang/hapus.php';
				break;
		
			case 'eks':
				include 'gudang/eks.php';
				break;
...
?>
	...
	<p class="h5">Tabel Logistik (Online)</p>
	<hr class="my-2">
			    ...
			    <!-- items per page dropdown -->            						
                        ...
                        <div 
						   ...
						   <button class="btn btn-primary"   onclick="location.href='./index.php?hlm=gudangon&aksi=baru'">Tambah Data</button>
					</div>		
 <!-- data --> 
...
$result=mysqli_query($con, "SELECT * FROM gudang_on INNER JOIN spa_on
				 ON gudang_on.nolkppgdg_on=spa_on.nolkpp_on
				 INNER JOIN admjual_on ON admjual_on.nolkppadm_on = spa_on.nolkpp_on
				 INNER JOIN produk_master ON produk_master.id_prod = gudang_on.idprodgdg_on
				 INNER JOIN distributor ON distributor.iddsb=gudang_on.pabrikgdg_on
				ORDER BY nolkppgdg_on + 0 DESC");		
		 ...
		 while($row = mysqli_fetch_array($result)){
                            ...
                            <tbody>                      
                                <tr class=tbl-item>
									...
									<td class=nosj>'.$row['nopo_on'].'</td>
									<td class=nosj>'.$row['nosj_on'].'<br>
									<a href="?hlm=gudangon&aksi=eks&nolkppgdg_on='.$row["nolkppgdg_on"].'"  ><i class="fa fa-truck"></i></a></td>
									<td class=nolkpp >'.$row['nolkppgdg_on'].'</td>
								   ...
								   <td><a href="?hlm=gudangon&aksi=edit&nolkppgdg_on=<?=$row["nolkppgdg_on"]; ?>" class="cus-application_form_edit"></a> |
								  	<a href="#" type="button" class="cus-cross" data-toggle="modal" data-target="#myModal<?php echo $row["nolkppgdg_on"] ?>"></a>  </td>					
								</tr>
								
								
								
								     <div class="modal fade" id="myModal<?php echo $row["nolkppgdg_on"]; ?>" role="dialog"  tabindex="-1" >
              <div class="modal-dialog">
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form role="form" action="gudang\hapus.php" method="get">

                        <?php
                        $nolkppgdg_on = $row["nolkppgdg_on"]; 
                        $query_edit = mysqli_query($con,"SELECT * FROM gudang_on WHERE nolkppgdg_on='$nolkppgdg_on'");
                        //$result = mysqli_query($conn, $query);
                        while ($rows = mysqli_fetch_array($query_edit)) {  
                        ?>

                        <input type="hidden" name="nolkppgdg_on" value="<?php echo $rows["nolkppgdg_on"] ?>" >
                       <input type="hidden" name="username" value="<?php echo $_SESSION['username'] ?>" >

                      <div class="form-group">
						<label for="message-text" class="col-form-label">Alasan LKPP <b><?php echo $rows["nolkppgdg_on"] ?></b> dihapus ?</label>
