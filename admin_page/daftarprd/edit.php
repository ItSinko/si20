...
<legend class="legend_form">Data Produk</legend>
   ...
   <div class="form-group row">
    ...
    <div class="col-sm-2">
    <input type="text" class="form-control"  placeholder="-" value="<?php echo $prd['sing_prod']; ?>"  id="type" >
    </div>
    </div>
 ...
 <div class="form-group row">
    ...
    <div class="col-sm-3">
    <textarea class="form-control" rows="1" minlength="6" placeholder="Minimal isi 5 karakter..." id="ket_log" ></textarea>
    </div>
    </div>
...
<button type="submit" class="btn btn-success" style="position: absolute; right:55%;"  onclick="return confirm('Apakah data sudah benar ?');" id="tambah" name="tambah" >Update Data</button>	
 ...
 <a href="./index.php?hlm=dafprd" formnovalidate  onclick="return confirm('Kembali ?');"  style="position: absolute; right:45%;"  class="btn btn-danger">Batal</a>
 </div>
...
$("#check").submit(function(e) {
	...
	if(id_prod == "" || type == ""|| nmprd == ""|| merk == ""|| kategori == "" ||  ket_log == "") {
		$("#error_message").show().html("Wajib mengisi kolom bertanda *");
	} else {
		$("#error_message").html("").hide();
		$.ajax({
			type: "POST",
			url: "check_form/edit/dafprd.php",
			data: "id_prod="+id_prod+"&type="+type+"&nmprd="+nmprd+"&merk="+merk+"&kategori="+kategori+"&ket="+ket+"&ket_log="+ket_log+"&username="+username,
